...
def learn(state, action, reward, next_state):
    ...
    """
    new_max_q = max([q.get((next_state, a), 0.0) for a in actions]) if next_state else 0
...
avg_steps_per_episode, timesteps_of_episode, reward_of_episode = run()

episode_rewards = np.array(reward_of_episode)
